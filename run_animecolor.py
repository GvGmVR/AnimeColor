...
# ============================================================
# 2. PATHS  — edit these to match your setup
# ============================================================
test = 5
...
OUTPUT_DIR = BASE_PATH / "outputs" / f"Test{test}"
...
# 3. TUNABLE SETTINGS
# ============================================================
START_FRAME  = 50      # Change to process a different 49-frame window
NUM_FRAMES   = 49     # Native CogVideoX-2B context — do not exceed this
WIDTH        = 512
HEIGHT       = 320
OUTPUT_FPS   = 24     # 24 = anime standard  |  12 = slower/smoother feel
GUIDANCE_SCALE   = 9   # raise to 8-9 for stronger reference adherence
INFERENCE_STEPS  = 25    # 25 is the paper default; 30 for slightly better quality
INFER_REST   = 20     # seconds to rest GPU after inference
DECODE_REST  = 15     # seconds to rest GPU after decode
SEED         = 42
...
if not SRC_DIR.exists():
    raise FileNotFoundError(f"Missing source code at {SRC_DIR}")
...
# ============================================================
# 15. NEXT-SEGMENT HINT
#     Print the command for the next 49-frame window so you can
#     chain segments manually and stitch them in a video editor.
# ============================================================
next_start = START_FRAME + actual_frames
if next_start < available:
    remaining = available - next_start
    print(f"\n  To process the next segment, set START_FRAME = {next_start}")
    print(f"  ({remaining} frames remain in {LINEART_DIR})")
else:
    print(f"\n  All {available} available frames have been processed.")